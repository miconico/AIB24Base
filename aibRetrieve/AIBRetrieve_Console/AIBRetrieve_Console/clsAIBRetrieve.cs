...
namespace AIBRetrieve_Console
{
    ...
    public class AIBWebSession
    {
        ...
        public string sParseAccountSMS()
        {
            ...
            try
            {
                ...
                while (sParseHTML.Contains("<span>"))
                {
                    ...
                    if (sSpanData.Contains("-"))
                    {
                        ...
                        sSpanData += "::" + sParseHTML.Substring(sParseHTML.IndexOf("<h3>", iStartPoint) + 4, (sParseHTML.IndexOf("</h3>", iStartPoint) - sParseHTML.IndexOf("<h3>", iStartPoint) - 4)).Replace("\t", "").Replace("\r", "").Replace("\n", "").Replace("&nbsp;", "");

                        arrAccounts.Add(sSpanData);
